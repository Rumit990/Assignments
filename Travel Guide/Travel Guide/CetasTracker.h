...
/*!
 * @brief : Setup configuration object and api key for the tracker object. 
 *
 ...
 *  Prior to calling this, Config Object must created with all user information and configuration flag set(for example update inteval).
 * @code:
 *  
 - (void)applicationDidFinishLaunching:(UIApplication *)application
 {
   //....
    ...
    //Setup configuration object and api key for the tracker object. 
     [[CetasTracker getDefaultTracker] startTrackerWithApiKey:@"apiKey" config:config];
   //...
}
 
 * @params
 *   apikey : The API key for your Cetas App project.
 *   config : Configuration object having all user information and configuraion flags set.
 */



- (void)startTrackerWithApiKey:(NSString *)apiKey  config :(CetasConfig *)config;
 ...
 * @brief : This method allows you record event in a single call. Developer don't need to create a event object in this call.
  * The Events are stored on to buffer until capacity and update interval as set in config object are not exceeded.
 * @code:
 ...
 * Event : Event Object with all its parameters set like details(event detail),date and time(optional).
 */
- (void)updateEvent:(CetasEvent *)event;

/*!
 * @name : Send an multiple event information to service forcefully.
 * @brief : This method updates the custom events to cetas service surpassing the criteria of capacity/update interval. As Soon as it is called,  events are updated on Cetas Server.In case internet is not connected events are stored on to buffer.
 *
 *  This method allows you to specify custom events within your app. Usually developers should log events related to navigation or page view, any user action, and other events as they are applicable to tracking progress towards their business goals.
 * @code
    //Initialize event object.
